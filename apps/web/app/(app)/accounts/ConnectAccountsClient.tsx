...
const CATEGORY_META: Record<string, { label: string; color: string; bg: string; border: string; dot: string }> = {
  live: { label: 'Live',  color: 'var(--green)',     bg: 'rgba(0,214,143,.1)',  border: 'rgba(0,214,143,.25)',  dot: 'var(--green)'      },
  demo: { label: 'Demo',  color: 'var(--eb-cyan)',   bg: 'rgba(6,182,212,.1)',  border: 'rgba(6,182,212,.25)',  dot: 'var(--eb-cyan)'    },
  prop: { label: 'Prop',  color: 'var(--eb-purple)', bg: 'rgba(139,92,246,.1)', border: 'rgba(139,92,246,.25)', dot: 'var(--eb-purple)'  },
};

const CATEGORY_ORDER: string[] = ['live', 'demo', 'prop'];
const CATEGORY_LABELS: Record<string, string> = { live: 'Live accounts', demo: 'Demo / paper', prop: 'Prop / funded' };
    ...
    <>
      ...
      <div style={{ padding: '18px 26px 60px', maxWidth: 960, width: '100%', alignSelf: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
          {[1, 2].map(k => (
            <div key={k} style={{ background: 'var(--eb-panel)', border: '1px solid var(--eb-border)', borderRadius: 12, padding: 18 }}>
...
function EmptyState({ onConnect }: { onConnect: () => void }) {
  return (
    <div style={{ padding: '60px 24px', textAlign: 'center', maxWidth: 520, margin: '0 auto' }}>
      ...
      {/* Supported exchanges */}
      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--eb-muted)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 14 }}>
        Supported exchanges
      </div>
        ...
        {[
          ...
          return (
            <div key={id} style={{
              ...
              <Image src={logo} alt={label} width={72} height={18} style={{ objectFit: 'contain' }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, textAlign: 'left' }}>
        {[
          { Icon: ShieldCheck, color: 'var(--green)',    bg: 'rgba(0,214,143,.1)',  title: 'Read-only',       desc: 'API keys are tested for withdrawal access on save and rejected if they have it.' },
          { Icon: KeyRound,    color: 'var(--eb-purple)', bg: 'rgba(139,92,246,.1)', title: 'Encrypted at rest', desc: 'Keys are envelope-encrypted with AWS KMS. Plaintext only lives in worker memory.' },
          { Icon: Zap,         color: 'var(--eb-cyan)',   bg: 'rgba(6,182,212,.1)',  title: 'Auto-sync',        desc: 'Trades are synced continuously in the background — no manual imports needed.' },
          ...
          <div key={title} style={{
            ...
            <div style={{ width: 28, height: 28, borderRadius: 8, background: bg, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
              <Icon size={14} style={{ color }} />
            </div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--eb-text)', marginBottom: 4 }}>{title}</div>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
        <div style={{
          ...
          {logo ? (
            <Image src={logo} alt={account.venue} width={26} height={26} style={{ objectFit: 'contain' }} />
          ) : (
            <span style={{ fontSize: 14, fontWeight: 700, color }}>{account.venue[0]?.toUpperCase()}</span>
          )}
        </div>

        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--eb-text)', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {account.label}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 11, fontWeight: 600, color, padding: '1px 7px', borderRadius: 99, background: bg }}>
              {account.venue.charAt(0).toUpperCase() + account.venue.slice(1)}
            </span>
            <span style={{ fontSize: 11, fontWeight: 600, color: catColor, padding: '1px 7px', borderRadius: 99, background: catBg, border: `1px solid ${catBorder}` }}>
              {catLabel}
            </span>
            <span style={{ fontSize: 11, color: 'var(--eb-muted)' }}>{account.accountType} · {account.baseCurrency}</span>
          </div>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--eb-muted)', padding: 4, borderRadius: 6, display: 'flex' }}
          >
...
export default function ConnectAccountsClient() {
  ...
  return (
    <div style={{ padding: '18px 26px 60px', maxWidth: 960, width: '100%', alignSelf: 'center' }}>
      ...
      {/* breadcrumb */}
      <div style={{ color: 'var(--eb-muted)', fontSize: 12, marginBottom: 6 }}>Workspace / Accounts</div>
      ...
      {hasAccounts ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          {CATEGORY_ORDER
            .filter(cat => (accounts ?? []).some(a => a.category === cat))
            .map(cat => {
              ...
              return (
                <div key={cat}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--eb-muted)', textTransform: 'uppercase', letterSpacing: '.07em' }}>
                      {CATEGORY_LABELS[cat]}
                    </span>
                    <span style={{ fontSize: 11, fontWeight: 600, color: meta.color, padding: '1px 7px', borderRadius: 99, background: meta.bg, border: `1px solid ${meta.border}` }}>
                      {catAccounts.length}
                    </span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
